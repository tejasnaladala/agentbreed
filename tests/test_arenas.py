...
class TestCodingArenaEvaluateCorrect:
    """Tests for CodingArena.evaluate with correct code."""

    async def test_correct_code_passes(self) -> None:
        """Agent returning correct is_palindrome should get high fitness."""
        correct_code = '''```python
...
```'''
        arena = CodingArena()
        # Only use the palindrome problem
        arena.problems = [_BUILTIN_PROBLEMS[0]] if hasattr(arena, "problems") else arena.problems
        # Reassign to just palindrome
        arena.problems = [p for p in arena.problems if p["function_name"] == "is_palindrome"]

        genome = _make_genome()
        adapter = _make_mock_adapter(correct_code)
        tasks = await arena.generate_tasks(count=1, seed=0)

        result = await arena.evaluate(genome, adapter, tasks)

        assert result.fitness_score == 1.0
        assert result.fitness_breakdown["pass_rate"] == 1.0

    async def test_correct_fibonacci(self) -> None:
        correct_code = '''```python
...
```'''
        arena = CodingArena(
            problems=[p for p in _BUILTIN_PROBLEMS if p["function_name"] == "fibonacci"]
        )
        genome = _make_genome()
        adapter = _make_mock_adapter(correct_code)
        tasks = await arena.generate_tasks(count=1, seed=0)
...
class TestCodingArenaEvaluateWrong:
    """Tests for CodingArena.evaluate with wrong code."""

    async def test_wrong_code_fails(self) -> None:
        """Agent returning wrong code should get low/zero fitness."""
        wrong_code = '''```python
def is_palindrome(s: str) -> bool:
    return False
```'''
        arena = CodingArena(
            problems=[p for p in _BUILTIN_PROBLEMS if p["function_name"] == "is_palindrome"]
        )
        genome = _make_genome()
        adapter = _make_mock_adapter(wrong_code)
        tasks = await arena.generate_tasks(count=1, seed=0)

        result = await arena.evaluate(genome, adapter, tasks)

        # "racecar" is palindrome, "" is palindrome, "A man a plan..." is palindrome
        # Always returning False means those 3 fail; "hello" and "ab" pass -> 2/5 = 0.4
        assert result.fitness_score < 1.0

    async def test_syntax_error_code_fails(self) -> None:
        bad_code = "```python\ndef is_palindrome(s) return True\n```"
        arena = CodingArena(
            problems=[p for p in _BUILTIN_PROBLEMS if p["function_name"] == "is_palindrome"]
        )
        genome = _make_genome()
        adapter = _make_mock_adapter(bad_code)
        tasks = await arena.generate_tasks(count=1, seed=0)

        result = await arena.evaluate(genome, adapter, tasks)
        assert result.fitness_score == 0.0

    async def test_adapter_error_gives_zero(self) -> None:
        arena = CodingArena()
        genome = _make_genome()
        adapter = _make_error_adapter()
        tasks = await arena.generate_tasks(count=3, seed=42)
...
# ---------------------------------------------------------------------------
# Need to import _BUILTIN_PROBLEMS for test setup above
# ---------------------------------------------------------------------------
from breed.arenas.coding import _BUILTIN_PROBLEMS  # noqa: E402
